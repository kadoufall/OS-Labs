...
page_init(void)
{
	...
	page_free_list = NULL;
	for (i = 0; i < npages; i++) {
		int tem = ((uint32_t)boot_alloc(0) - KERNBASE) / PGSIZE + (EXTPHYSMEM - IOPHYSMEM) / PGSIZE;
		if(i==0){
			pages[i].pp_ref = 1;
		} else if(i>=npages_basemem && i<npages_basemem+tem){
			pages[i].pp_ref = 1;
		} else{
			pages[i].pp_ref = 0;
...
page_alloc(int alloc_flags)
{
	...
	struct Page *re = page_free_list;
    page_free_list = re->pp_link;
    re->pp_link = NULL;
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if(!(pgdir[dicIndex] & PTE_P)){				// page table doesn't exixt	
		if(!create){
			return NULL;
		}
		
		struct Page* newPage = page_alloc(1);		// create new page
		if(newPage == NULL){
			return NULL;
		}
		newPage->pp_ref++;
		pgdir[dicIndex] = page2pa(newPage) | PTE_P | PTE_W | PTE_U;	
	}
	
	pte_t *pageTable = KADDR(PTE_ADDR(pgdir[dicIndex]));
...
page_insert(pde_t *pgdir, struct Page *pp, void *va, int perm)
{
	...
	pp->pp_ref++;					// before remove to avoid pp is freed before inserted
	if((*entry) & PTE_P){
...
page_lookup(pde_t *pgdir, void *va, pte_t **pte_store)
{
	...
	struct Page *re = pa2page(PTE_ADDR(*entry));
	if(pte_store != NULL){
		*pte_store = entry;	
	}
...
page_remove(pde_t *pgdir, void *va)
{
	// Fill this function in
	pte_t *entry = NULL;
	struct Page *page = page_lookup(pgdir, va, &entry);
...
{
	// LAB 3: Your code here.
	char *first = ROUNDDOWN((char *)va, PGSIZE);
	char *last = ROUNDUP((char *)(va + len), PGSIZE);
	char *i;	
	for(i=first; i<last; i+=PGSIZE){
		pte_t *temPTE = pgdir_walk(env->env_pgdir, (void *)i, 0);
		if(((int)i>=ULIM) || (temPTE==NULL) || (!(*temPTE & PTE_P)) || ((*temPTE & perm) != perm)){
