...
sys_exofork(void)
{
	...
	struct Env* env;
	int re = env_alloc(&env, sys_getenvid());
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	int re = page_insert(env->env_pgdir, page, va, perm);
	if(re < 0){
		page_free(page);
		return -E_NO_MEM;
	}
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	int re = page_insert(dstEnv->env_pgdir, page, dstva, perm);
	if(re != 0){
		return -E_NO_MEM;
	}
...
static int
sys_env_change_priority(envid_t envid, uint32_t priority)
{
	struct Env *env;
